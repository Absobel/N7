...
procedure Evaluer_Alea_LCA is
	...
	is
		...
		function Sda_Min is new Extremum(Relation_Ordre => Inferieur);

		function Superieur (A, B: Integer) return Boolean is
		begin
			return (A > B);
		end;
		function Sda_Max is new Extremum(Relation_Ordre => Superieur);

		Sda: T_LCA;               -- Liste chainée des fréquences
		Nombre_alea: Integer;     -- nombre aléatoire
		Donnee: Integer;          -- Variable qui retient la donnee du nombre aléatoire pour l'incrémenter
	begin
		Initialiser(Sda);
		for i in 1 .. Borne loop             -- Initialiser la liste avec des fréquences nulles
			Enregistrer(Sda, i, 0);
		end loop;
		for i in 1 .. Taille loop                         -- Tirer les nombres aléatoires
			Get_Random_Number(Nombre_alea);
			Donnee := La_Donnee(Sda, Nombre_alea);
			Enregistrer(Sda, Nombre_alea, Donnee + 1);
		end loop;
		Min := Sda_Min(Sda);
		Max := Sda_Max(Sda);
		Vider(Sda);
