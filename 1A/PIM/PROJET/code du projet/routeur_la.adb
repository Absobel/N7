...
procedure routeur_la is
    ...
    Destination_Defaut : constant unbounded_String := To_Unbounded_String("Erreur");
    package cache_la_routeur is new cache_la (Destination_Defaut);
    use cache_la_routeur;
    ...
    Politique : T_Politique;    -- La politique de gestion du cache (transfomation de Argument_Politique)
            ...
            -- Traitement de la commande
                if commande="table" then
                    ...
                    if Afficher_Statistiques then
                        ...
                        if Taille_Cache = 0 then
                            Put_Line("Statistiques (ligne " & Integer'Image(Numero_Ligne) & ") :");
                            Put_Line("Cache désactivé");
                            new_Line;
                        else
                            Statistiques(Cache, Numero_Ligne); New_Line;
                        end if;
                    else
                        Put_Line("Statistiques (ligne " & Integer'Image(Numero_Ligne) & ") :");
