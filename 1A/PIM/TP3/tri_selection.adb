...
procedure Tri_Selection is
    ...
    -- Objectif : Trier un tableau suivant le tri par insertion
    -- Paramètres :
    --     Tab : Tableau à trier
    -- Précondition :
    --     La taille du tableau doit être inférieure à Capacite
    -- Resultat : Le tableau sera trié
    procedure tri_selection(Tab: in out T_Tableau) with
        Pre => Tab.Taille <= Capacite
    is
        Idx_Min: Integer;
        Temp_Echange: Integer; -- Variable permettant d'aider à permuter deux variables d'un tableau
    begin
        for idx in 1..Tab.Taille loop
            -- trouver l'index du minimum de idx à Tab.Taille
            Idx_Min := idx;
            for idx2 in idx..Tab.Taille loop
                if Tab.Elements(idx2) < Tab.Elements(Idx_Min) then
                    Idx_Min := idx2;
                else
                    null;
                end if;
            end loop;
            Temp_Echange := Tab.Elements(idx);
            Tab.Elements(idx) := Tab.Elements(Idx_Min);
            Tab.Elements(Idx_Min) := Temp_Echange;
        end loop;
    end tri_selection;
