...
package body Vecteurs_Creux is
	...
	begin
		...
		if Est_Nul(V2) or else V.ALL.Indice > Indice then
			return 0.0;
		else -- V2.All.Indice = Indice then
	...
	begin
		if Est_Nul(V) or else V.All.Indice > Indice then
			...
			if Valeur /= 0.0 then
				...
				V := V.All.Suivant;   -- Perte de mémoire TODO
			end if;
		else
