...
rule scanner = parse
  ...
  | "(" {UL_LEFT_BRACKET::(scanner lexbuf)}
  | ")" {UL_RIGHT_BRACKET::(scanner lexbuf)}
  | ";" {UL_SEMI_COLUMN::(scanner lexbuf)}
  ...
  | majuscule alphabet* as name {(UL_INTERFACE_INDENT name)::(scanner lexbuf)}
  | eof							{ [UL_FIN] }
