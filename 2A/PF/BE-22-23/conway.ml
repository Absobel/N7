...
let max = fun l ->  List.fold_right (fun an init -> if an > init then an else init) l 0
...
let rec suite  = fun taille terme ->
    if taille = 1 then
        [terme]
    else
        let suiv = suivant terme in
        terme::(suite (taille-1) suiv)
...
(* Remarque : TO DO *)
(* Il n'est pas possible de prouver cette propriété, on ne peut que calculer de plus en plus de termes *)