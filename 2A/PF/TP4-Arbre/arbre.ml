...
let rec retrait_arbre lc (Noeud (b, lb)) =
   ...
   | [] -> Noeud (false, lb)
   | c::qlc ->
      let some_arbre_c = recherche c lb in
      match some_arbre_c with
      | None -> Noeud (b, lb)
      | Some arbre_c -> Noeud(b, maj c (retrait_arbre qlc arbre_c) lb)
...
let%test _ = retrait_arbre ['a';'b'] arbre_sujet4 = arbre_sujet_5
let%test _ = retrait_arbre ['a';'c'] arbre_sujet4 = arbre_sujet_6
let%test _ = retrait_arbre ['a';'d'] arbre_sujet4 = arbre_sujet4

(******************************************************************************)
(*   fonction qui liste les éléments d'un arbre dans leur                     *)
(*   ordre lexicographique                                                    *)
(*   signature  : liste_elements : 'a arbre -> 'a list list                   *)
(*   paramètre  : un arbre n-aire                                             *)
(*   résultat   : la liste des éléments de l'arbre                            *)
(******************************************************************************)

let parcours (Noeud(_, lf)) =
   let rec dico lf =
      match lf with
      | [] -> []
      | (c, Noeud(b, lff))::qlf ->
...
(******************************************************************************)
(*   fonction qui supprime les branches inutiles d'un arbre                   *)
(*   signature  : nettoyage : 'a arbre -> 'a arbre                            *)
(*   paramètre  : un arbre n-aire                                             *)
(*   résultat   : l'arbre n-aire nettoyé                                      *)
(******************************************************************************)

let rec nettoyage (Noeud(b, lb)) =
   match lb with
   | [] -> Noeud(b, []) (* ça n'arrive que dans le cas de l'arbre vide ou de si b est true normalement *)
   | _ -> Noeud(b, List.map (fun (c,a) -> (c, nettoyage a)) (List.filter (
