...
let rec parse_P : (char, prog) parser = fun flux ->
  (
    return flux >>= fun _ -> p_begin >>= fun _ -> parse_I >>= fun i -> p_end >>= fun _ -> return i
  ) flux
...
let rec debug_flux flux =
  match Flux.uncons flux with
  | None        -> failwith "Parsing failed"
  | Some (c, q) -> Format.printf "%c" c; debug_flux q

(* let _ = debug_flux (flux_of_string "begin repeat 2 move 10; turn 90; end") *)
(* let _ = debug_flux (flux_of_file "/home/absobel/Mir/GitRepos/N7/2A/PF2/TP10-Parser/test.logo") *)

let rec run_flux_prog flux =
  match Solution.uncons flux with
  | None        -> failwith "Parsing failed"
  | Some (p, q) -> run_logo p; run_flux_prog q

let _ = flux_of_string "begin repeat 2 move 10; turn 90; end" |> parse_logo |> run_flux_prog
(* let _ = flux_of_file "/home/absobel/Mir/GitRepos/N7/2A/PF2/TP10-Parser/test.logo" |> parse_logo |> run_flux_prog *)

(* let main_logo fn =
  let f = flux_of_file fn in
  match Solution.uncons (parse_logo f) with
  | None        -> failwith "Parsing failed"
  | Some (p, q) -> run_logo p; ()

let _ = main_logo "/home/absobel/Mir/GitRepos/N7/2A/PF2/TP10-Parser/test.logo" *)