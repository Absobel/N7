...
module Fr : Frame = struct
  let dt = 0.01
  let box_x = (0.0, 640.0)
  let box_y = (0.0, 480.0)
end

module OurFreeFall = FreeFall(Fr)
module OurDrawing = Drawing(Fr)

(* let _ = 
  let init = ((320.0, 400.0), (40.0, 0.01)) in
  OurDrawing.draw (OurFreeFall.run init) *)
...
module OurBouncing = Bouncing(Fr)

let _ = 
  let init = ((320.0, 400.0), (40.0, 0.01)) in
  OurDrawing.draw (OurBouncing.run init)