...
let retrait mot (Trie(arbre, decompose, recompose)) = 
  Trie (nettoyage (retrait_arbre (decompose mot) arbre),decompose,recompose)
...
let affiche p (Trie(arbre, _, recompose_chaine)) =
  let list_mot = parcours arbre in
  List.iter p (List.map recompose_chaine list_mot)